...
class UpdateState extends BaseHelper {
  ...
  _getExecutionUpdatedFields(counts, commandsCount) {
    ...
    if (finishedStates >= commandsCount) {
      if (states.canceled > 0) {
        updatedExecutionFields.states = 'canceled';
      } else if (states.timeout > 0) {
        updatedExecutionFields.states = 'timeout';
      } else {
        updatedExecutionFields.states = 'finished';
      }
    } else if (states.running && (states.running - (states.finished + states.canceled)) > 0) {
      updatedExecutionFields.states = 'running';
    } else {
      updatedExecutionFields.states = 'pending';
    }
  ...
  _onUpdateExecution(context, execution) {
    const that = this;
  ...
  _onGetInstancesStates(context, execution, instancesStates) {
    const that = this;
    const allHaveAnswered = execution.instances.length === instancesStates.length;
    const noneIsPending = instancesStates.every(
      (instancesState) => instancesState.state !== 'pending'
    );
    if (allHaveAnswered && noneIsPending) {
      const deleteScheduleJob = {
        ...
        payload: {
          url: nodeUrl.resolve(that.schedulerApiUrl,
            `schedules/${execution.pendingTimeoutScheduleId}`),
        },
      };
