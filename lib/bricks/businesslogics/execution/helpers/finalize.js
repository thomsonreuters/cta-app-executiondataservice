...
class Finalize extends BaseHelper {
  ...
  _onFindExecution(context, execution) {
    ...
    const isFinishedState = ['finished', 'canceled'].indexOf(execution.state) !== -1;
    if (execution.completed !== isFinishedState && isFinishedState) {
  ...
  _onGetStatesIndex(context, execution, resultsIndexes, statesIndexes) {
    ...
    const noMissingResults = keys.every(function(key) {
      ...
      let noMissing = false;
      if (hasKey) {
        noMissing = resultsIndexes[key].maxidx === statesIndexes[key].maxidx;
      }
      return hasKey && noMissing;
    });
    if (noMissingResults) {
      const updateFields = {
        completed: true,
