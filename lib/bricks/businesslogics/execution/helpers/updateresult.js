...
class UpdateResult extends BaseHelper {
    ...
    findExecutionContext.on('done', function(brickname, execution) {
      ...
      getResultsCountContext.on('done', function(bricknameOne, count) {
        if (count >= execution.nbresults) {
          ...
          getResultsAggregationContext.on('done', function(bricknameTwo, response) {
            const updateFields = that._getExecutionUpdatedFields(response, count);
  ...
  _getExecutionUpdatedFields(aggregation, count) {
    ...
    updatedExecutionFields.nbresults = count;
