...
class UpdateOne extends BaseDBInterfaceHelper {
  ...
  _process(context) {
    ...
    const mongoDbFilter = {
      _id: new ObjectID(context.data.payload.content.id),
    };
    const mongoDbDocument = {
      $set: _.omit(context.data.payload.content, ['id']),
    };
    ...
    output.on('done', function(brickname, response) {
      if (response.ok) {
        if (response.hasOwnProperty('value') && response.value !== null) {
          const object = _.omit(response.value, '_id');
          object.id = response.value._id.toString();
